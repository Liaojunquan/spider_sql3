...
select_last_sql = "select episodes,createtime from series where imdb_id=? order by createtime desc limit 2;"
...
def download_subtitles_file(url, language, imdb, tv_name, subtitle_id):
    ...
    for srt_file_name in zip_file.namelist():     # 遍历压缩包内所有文件的文件名称
        ...
        if '.' in srt_file and not srt_file.endswith('.srt') and not srt_file.endswith('.ass') and \
            ...
            is_error_file = True
        srt_file_tmp = srt_file.replace('.ass', ' ').replace('.smi', ' ').replace('.sub', ' ').replace('.ssa', ' ').replace('.txt', ' ').replace('.srt', ' ').replace('  ', ' ')
        try:
            season_and_episode = regexp_function.get_season_and_episode(srt_file_tmp, tv_name)  # 从文件名获取季和集信息
        except Exception as err:
            print_and_log('Get season and episode Error!  ' + srt_file + '  <<' + tv_name + '>>')
            traceback.print_exc()
            print_and_log(err)
            season_and_episode = False
        if season_and_episode is None:
            try:
                episode = regexp_function.get_episode(srt_file_tmp, tv_name)  # 从文件名获取集信息
            except Exception as err:
                print_and_log('Get episode Error!  ' + srt_file + '  <<' + tv_name + '>>')
                traceback.print_exc()
                print_and_log(err)
                episode = None
            if episode is None:

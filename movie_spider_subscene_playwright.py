    ...
    while count < 3:
        ...
        try:
            ...
            if 'To many requests' in page.title():
                print_and_log('Warning: To many requests! 326')
                time.sleep(15)
                count += 1
                continue
            time.sleep(5)
...
def search_movies(mv_name, imdb_list, imdb_list_index):
    global last_time, playwright, browser, page, last_restart_time
    ...
    if '<input type="text" name="query" id="query"' not in source_code or \
        ...
        if count >= 3:
            ...
            return None
    try:
        page.fill("//input[@id='query']", mv_name)
        page.click("text=Subtitle Search")
    except Exception as err:
        traceback.print_exc()
        print_and_log(err)
        print_and_log(f'Error: Unable to search {mv_name}!')
        return False
    ...
    while li_index < len(lis):
        ...
        if mv_url in url_set:           # 检测该url是否已被访问过
            li_index += 1
            continue
        ...
        while count < 3:
            ...
            try:
                ...
                print_and_log(err)
                print_and_log(f'Error: Unable open page {mv_url}, retry again {count}')
                time.sleep(10)
                count += 1
            else:
                if 'To many requests' in page.title():
                    print_and_log('Warning: To many requests! 663')
                    time.sleep(15)
                    count += 1
                else:
                    break
        if count >= 3:
            print_and_log(f'Error: Unable open page {mv_url}, {imdb_list[imdb_list_index]}, ignore!')
            li_index += 1
            continue
        ...
        if a_s is None or len(a_s) == 0:
            print_and_log(f'Error: {page.title()}')
            print_and_log(f'Error: No subtitles list in page {mv_url}')
            li_index += 1
            continue
        ...
        while a_index < len(a_s):  # 遍历字幕列表每一个字幕
            ...
            while count < 3:
                ...
                try:
                    ...
                    if 'To many requests' in page.title():
                        print_and_log('Warning: To many requests! 745')
                        time.sleep(15)
                        count += 1
                    else:
                        break
            ...
            if download_url is None or len(download_url) == 0:
                print_and_log(f'Error: Unable get subtitles download url from {file_page_url}, {imdb_list[imdb_list_index]}, <<{mv_name}>>')
                a_index += 1
                continue
